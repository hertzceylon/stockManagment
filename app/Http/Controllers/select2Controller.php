...
class Select2Controller extends Controller
{
    ...
    public function GetData($id=0,$column,$table,$term,$id_to_select)
    {
        $data=NULL;
        switch ($id) {
            case 0://general//
        ...
        if($data){
            return $data->orWhere(DB::raw('CONCAT('.$column.')'), 'LIKE', '%' . $term . '%')->get([$id_to_select.' as id', DB::raw('CONCAT('.$column.') as value')]);
        }
    }
}